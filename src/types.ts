...
export type HyprlandIPCEventsMap = {
  ...
  // Native events
  'hyprland.startEmitted': []
  'hyprland.shutdownEmitted': []
  'window.open': [address: string]
  'window.open_early': [address: string]
  'window.close': [address: string]
  'window.destroy': [address: string]
  'window.kill': [address: string]
  'window.active': [address: string, focusReason: string]
  'window.urgent': [address: string]
  'window.title': [address: string, title: string]
  'window.class': [address: string, className: string]
  'window.pin': [address: string, pinned: '0' | '1']
  'window.fullscreen': [address: string, fullscreen: string]
  'window.update_rules': [address: string]
  'window.move_to_workspace': [address: string, workspace: string]
  'layer.opened': [layerSurface: string]
  'layer.closed': [layerSurface: string]
  'monitor.added': [monitor: string]
  'monitor.removed': [monitor: string]
  'monitor.focused': [monitor: string]
  'monitor.layout_changed': []
  'workspace.active': [workspace: string]
  'workspace.special_active': [workspace: string, monitor: string]
  'workspace.created': [workspace: string]
  'workspace.removed': [workspace: string]
  'workspace.move_to_monitor': [workspace: string, monitor: string]
  'config.reloaded': []
  'config.props_refreshed': [executedAsScheduled: string]
  'keybinds.submap': [name: string]
  'screenshare.state': [active: string, type: string, name: string]
  'input.keyboard.key': [keycode: string, timestamp: string, state: string]
}
